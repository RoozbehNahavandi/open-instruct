...
class Args:
    ...
    outer_lr: float = 0.009
    """The outer learning rate for the optimizer"""
    ...
    for training_step in range(resume_training_step, args.num_training_steps + 1):
        ...
        with unwrap_model_for_generation(model, accelerator) as unwrapped_model:
            ...
            with torch.no_grad():
                ...
                ### MDPO START
                # MDPO: Use rewards = scores only
                actual_start = torch.arange(logprobs.size(0), device=logprobs.device)
                actual_end = torch.where(sequence_lengths_p1 < logprobs.size(1), sequence_lengths_p1, sequence_lengths)
            ...
            for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                ...
                for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                    with accelerator.accumulate(model):
                        ...
                        ### MDPO START
                        # Mirror Descent loss
                        logprob_diff = new_logprobs - mb_logprobs
                        mirror_descent_loss = -masked_mean(mb_advantage * logprob_diff, ~padding_mask[micro_batch_inds])

                        # KL regularization
                        ref_logprobs_batch = ref_logprobs[micro_batch_inds]
                        kl_div = masked_mean(new_logprobs - ref_logprobs_batch, ~padding_mask[micro_batch_inds])

                        # MDPO loss = mirror descent step + KL penalty
                        pg_loss = mirror_descent_loss + args.outer_lr * kl_div
                        ...
                        with torch.no_grad():
                            ...
                            approxkl = 0.5 * (logprob_diff**2).mean()
