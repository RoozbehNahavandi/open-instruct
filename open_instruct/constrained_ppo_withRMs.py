...
def main(args: Args, dataset_config: DatasetConfig, model_config: ModelConfig):
    ...
    proxy_points = torch.tensor([0.4, 0.32])
            ...
            for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                ...
                for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                    with accelerator.accumulate(model):
                        ...
                        if not args.squash_fn:
                            lagrange = lagrange_multipliers.view(-1, 1, 1).detach()
                        elif args.squash_fn == 'sigmoid':
                            lagrange = torch.sigmoid(lagrange_multipliers.detach()).view(-1, 1, 1)
                        elif args.squash_fn == 'tanh':
                            lagrange = torch.tanh(lagrange_multipliers.detach()).view(-1, 1, 1)
                        ...
                        torch.nn.utils.clip_grad_norm_([lagrange], 1)
                        ...
                        with torch.no_grad():
                            ...
                            local_metrics[22] = lagrange_loss.mean().item()
                            # local_me
