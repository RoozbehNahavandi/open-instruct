...
def main(args: Args, dataset_config: DatasetConfig, model_config: ModelConfig):
    ...
    print(f'data[0]: {train_dataset[0]}')
    ...
    for _ in range(args.num_train_epochs):
        for data in dataloader:
            # print(f'tokenizer chosen data: {tokenizer.batch_decode(data[INPUT_IDS_CHOSEN_KEY], skip_special_tokens=True)}')
            # print(f'tokenizer rejected data: {tokenizer.batch_decode(data[INPUT_IDS_REJECTED_KEY], skip_special_tokens=True)}')
            ...
            #         if accelerator.is_main_process:
            #             print(f"[WARNING] Evaluation failed at step {training_step}: {e}")
