...
def main(args: Args, dataset_config: DatasetConfig, model_config: ModelConfig):
    ...
    print(f'constraint reward models list: {constraint_reward_models_list}')
    ...
    for module in [model, ref_model, main_reward_model] + constraint_rm_list:
    ...
    proxy_points = torch.tensor([0.2295, 0.4838])
        ...
        with unwrap_model_for_generation(model, accelerator) as unwrapped_model:
            ...
            with torch.no_grad():
                ...
                for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                    ...
                    # print(type(unwrapped_constraint_value_models), 'xxxxxxxxxx', type(unwrapped_constraint_value_models[0]))
                    

                    _, score, _ = get_reward(
                        main_reward_model, postprocessed_query_response, tokenizer.pad_token_id, context_length
                    )
                    unwrapped_main_value_model = accelerator.unwrap_model(model).main_value_model
                    full_value, _, _ = get_reward(
                        unwrapped_main_value_model, query_response.clone(), tokenizer.pad_token_id, context_length
                    )
            ...
            for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                ...
                for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                    with accelerator.accumulate(model):
                        ...
                        lagrange = torch.sigmoid(lagrange_multipliers).view(-1, 1, 1).detach()  # Detach Lagrange multipliers
                        # Compute Lagrange multipliers with sigmoid to bound them between 0 and 1
                        n_constraints = 2
                        # print(f'mb_constraint_values.shape: {mb_constraint_values.shape}, mb_values.shape: {mb_values.shape}')
                        # print(f'mb_constraint_return.shape: {mb_constraint_return.shape}, mb_return.shape: {mb_return.shape}')
                        # print(f'constraints_advantages.shape: {constraints_advantages.shape}, mb_advantage.shape: {mb_advantage.shape}')
                        # print(type(lagrange), lagrange)
                        # print(f'mb_constraint_advantage: {mb_constraint_advantages.shape}, {type(mb_constraint_advantages)}')
                        # print(f'kl shape: {kl.shape}')

                        # print(f'an3.shape: {an3.shape}')
                        # print(f'lagrange.shape: {lagrange.shape}, mb_constraint_advantage.shape: {mb_constraint_advantages.shape}')
                        # print(f'first term shape: {an.shape}')
                        # print(f'second term shape: {an2.shape}')
                        # Calculate mixed advantages using the given formula
                        mixed_advantages = (n_constraints - lagrange.sum()) * mb_advantage + torch.sum(lagrange * mb_constraint_advantages, dim=0)
                        # print(f'mixed_advantages.shape: {mixed_advantages.shape}')
